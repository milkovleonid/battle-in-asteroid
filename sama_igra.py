...
class Player(sprite.Sprite):
    ...
    def update(self, left, right, up, platforms, sp_orug, r, f, drop, zomba, veapon, oboyma_0, oboyma_1, oboyma_2,
        ...
        self.collide(0, self.yvel, platforms, drop, zomba, korobk, zzyq)
        self.rect.x += (self.xvel)  # переносим свои положение на xvel
        self.collide(self.xvel, 0, platforms, drop, zomba, korobk, zzyq)
    def collide(self, xvel, yvel, platforms, drop , zomba, korobk, zzyq):
        for p in platforms:
        ...
        for z in zomba:
            if sprite.collide_rect(self, z):
                ...
                seconds = (pygame.time.get_ticks() - self.start_ticks1) / 500  # вычисляем сколько прошло секунд
                if seconds % 1 == 0:
                    self.health -= 2
        for ii in zzyq:
            if sprite.collide_rect(self, ii):
                self.health -= 10
                self.start_ticksz = pygame.time.get_ticks()
                seconds = (pygame.time.get_ticks() - self.start_ticks1) / 500  # вычисляем сколько прошло секунд
                if seconds % 1 == 0:
        ...
        for dropi in drop:
            if sprite.collide_rect(self, dropi):
                dropi.kill()
                dropi.remove(drop)
